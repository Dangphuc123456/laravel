...
function toggleChat(type) {
  const zaloChatbox = document.getElementById('zalo-chatbox');
  const facebookChatbox = document.getElementById('facebook-chatbox');

  if (type === 'zalo') {
      zaloChatbox.classList.toggle('hidden');
      facebookChatbox.classList.add('hidden'); // Hide Facebook chatbox if open
  } else if (type === 'facebook') {
      facebookChatbox.classList.toggle('hidden');
      zaloChatbox.classList.add('hidden'); // Hide Zalo chatbox if open
  }
}
